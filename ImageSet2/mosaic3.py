...
def homog_dlt(ptsa, ptsb):
    ...
    for i in range(len(ptsa)):
        u, v = ptsa[i].item(0), ptsa[i].item(1)
        u_p, v_p = ptsb[i].item(0), ptsb[i].item(1)
...
def homog_ransac(pair_pts_a, pair_pts_b):
    ...
    for i in range(n):
        ...
        for j in range(len(pair_pts_a)):
            b = pair_pts_a[j]
            #b = np.asarray([pair_pts_a[i].item(0), pair_pts_a[i].item(1), pair_pts_a[i].item(2)])
            #print 'b: ' + str(b)
            b_p = pair_pts_b[j]
            #b_p = np.asarray([pair_pts_b[i].item(0), pair_pts_b[i].item(1), pair_pts_b[i].item(2)])
            #print 'b_p: ' + str(b_p)
            #dist = cv2.norm(b, b_p, cv2.NORM_HAMMING)
            dist = np.linalg.norm(b - b_p)
            #print str(b[0])
            #dist = cv2.norm(np.asarray([b[0][0], b[1][0], b[2][0]]), np.asarray([b_p[0][0], b_p[1][0], b_p[2][0]]), cv2.NORM_HAMMING)
            if dist < threshold:
                inliers.append((b, b_p))
        if best_inliers == None or len(inliers) > len(best_inliers):
    ...
    return np.matrix(best_H), best_inliers_a, best_inliers_b
...
def perspect_combine(img_a, img_b, H, length, width):
    ...
    #cv2.imshow('warp_a', warp_a)
    mask_a = np.zeros((bw, bl, 3))
    for i in range(bw):
        for j in range(bl):
            if np.sum(warp_a[i][j]) > 0.:
                mask_a[i,j,:] = 1.
    #img_ab = np.float64(warp_a)
    img_ab = warp_a
    #cv2.imshow('img_ab', img_ab)
